...
class LangChainSettings(BaseAppSettings):
    ...
    api_key: str = Field(default="", alias="LANGCHAIN_API_KEY", description="LangChain API key")
    project: str = Field(alias="LANGCHAIN_PROJECT", default="local_turtle_app", description="LangChain project name")
    langsmith_api_key: str = Field(default="", alias="LANGSMITH_API_KEY", description="LangSmith API key")


class DataSettings(BaseAppSettings):
    movie_plots_path: Path = Field(
        default=Path("src/infrastructure/vector_store/data_pipeline/data/processed/wiki_movie_plots_cleaned.csv"),
        description="Path to the cleaned movie plots CSV file"
    )


class SMBSettings(BaseAppSettings):
    server: str | None = Field(alias="SAMBA_SERVER", description="SMB server IP or hostname")
    share_path: str | None = Field(alias="SAMBA_SHARE_PATH", description="SMB share path (e.g., /movies)")
    username: str | None = Field(alias="SAMBA_USER", description="SMB authentication username")
    password: str | None = Field(alias="SAMBA_PASSWORD", description="SMB authentication password")
...
class OpenSubtitlesSettings(BaseAppSettings):
    api_key: str = Field(default="", alias="OPENSUBTITLES_API_KEY", description="OpenSubtitles.com API key")
    username: str = Field(default="", alias="OPENSUBTITLES_USERNAME", description="OpenSubtitles.com account username")
    password: str = Field(default="", alias="OPENSUBTITLES_PASSWORD", description="OpenSubtitles.com account password")
