...
from infrastructure.smb.library_manager import LibraryManagerTool, scan_smb_movie_library
from core.utils.movie_names import extract_movie_metadata
...
def test_search_movies_fuzzy_match(library_manager_tool, mock_movies):
    """Test partial keyword match finds movies with one matching word."""
    # "judgment" is a full word in "Terminator 2 Judgment Day" — triggers tier 4 partial match
    results = library_manager_tool._search_movies(mock_movies, "judgment day", limit=5)
    assert len(results) > 0
...
@patch('infrastructure.smb.library_manager.scan_smb_movie_library')
...
def test_tool_run(library_manager_tool):
    ...
    with patch('infrastructure.smb.library_manager.scan_smb_movie_library') as mock_scan:
