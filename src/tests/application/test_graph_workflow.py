...
from core.utils.memory_utils import create_thread_id
...
class TestWorkflowToolSelection:
    ...
    def mock_components(self):
        ...
        with patch('application.workflows.graph.SupervisorNodeCreator') as mock_supervisor_class, \
             patch('infrastructure.llm.factory.create_agent_llm') as mock_agent_llm, \
             patch('application.agents.tool_agent.ToolAgent.process') as mock_agent_process:

            # The graph calls supervisor_node(state) — supervisor_node is the instance
            # returned by SupervisorNodeCreator(...). Set side_effect on the instance's
            # __call__ so mock_supervisor_instance.side_effect controls routing.
            mock_supervisor_instance = MagicMock()
            mock_supervisor_class.return_value = mock_supervisor_instance

            # Mock agent LLM
            mock_agent_llm.return_value = AsyncMock()

            # Mock agent process to route back to supervisor
            ...
            yield {
                ...
                'agent_llm': mock_agent_llm.return_value,
    ...
    def test_library_query_routes_to_library_scan(self, mock_components):
        ...
        with patch('infrastructure.smb.library_manager.library_manager_tool._run') as mock_library_tool:
...
class TestWorkflowExecution:
    ...
    def test_graph_initialization_has_all_agents(self):
        ...
        with patch('infrastructure.llm.factory.create_supervisor_llm'), \
             patch('infrastructure.llm.factory.create_agent_llm'):
            graph = create_movie_workflow()
